...
export function WhatsNext({ next, prevCompletedAt }: Props) {
  ...
  if (!next) {
    return (
      <motion.div
        ...
        className="bg-white border-2 border-[#D1FAE5] rounded-xl p-5 flex items-center gap-4"
      >
        <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#059669] text-white shrink-0">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="20 6 9 17 4 12" />
          </svg>
        </div>
        <div>
          <div className="text-sm font-semibold text-[#059669]">All 34 steps complete</div>
  ...
  return (
    <motion.a
      ...
      className="relative block cursor-pointer bg-white border-2 border-[#DBEAFE] rounded-xl p-5 hover:border-[#2563EB] transition-colors"
    >
      <div className="flex items-start justify-between mb-2.5 gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#2563EB]">
            Up next
          </span>
        </div>
        <span
      ...
      <div className="text-[17px] font-semibold text-[#111827] mb-1 leading-snug">
        {next.title}
      </div>
      {elapsed && (
        <div className="text-[12px] text-[#9CA3AF] font-medium mb-1.5">
      ...
      <div className="mt-3 inline-flex items-center gap-1.5 text-[13px] font-semibold text-[#2563EB]">
        Jump to step
